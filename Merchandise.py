...
class Merchandise:
    ...
    def promptMerchandiseSelection(self):
        ...
        selected_items = []
        while True:
            choice = input("Enter the item number to buy (or 'done' to finish, 'cancel' to abort): ").strip().lower()
            if choice == "cancel":
            ...
            qty_input = input(f"How many '{item_name}' would you like to buy? (type 'cancel' to go back): ").strip().lower()
            if qty_input == "cancel":
